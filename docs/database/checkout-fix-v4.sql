-- Checkout Database Fix Script v4
-- This script fixes the database issues preventing checkout from working
-- Handles existing tables with quantity_sold column and other legacy columns

-- ============================================================================
-- STEP 1: Fix Sales Table Schema - Remove Legacy Columns
-- ============================================================================
...
-- Remove legacy columns that are causing issues
DO $$
DECLARE
    has_quantity_sold BOOLEAN;
...
BEGIN
    ...
    IF has_quantity_sold THEN
        ALTER TABLE sales DROP COLUMN quantity_sold;
        RAISE NOTICE 'Removed quantity_sold column from sales table';
    END IF;
    
    IF has_product_id THEN
        ALTER TABLE sales DROP COLUMN product_id;
        RAISE NOTICE 'Removed product_id column from sales table';
    END IF;
    
    IF has_sale_price THEN
        ALTER TABLE sales DROP COLUMN sale_price;
        RAISE NOTICE 'Removed sale_price column from sales table';
    END IF;
    
    IF has_sold_by THEN
        ALTER TABLE sales DROP COLUMN sold_by;
        RAISE NOTICE 'Removed sold_by column from sales table';
    END IF;
END $$;
