  ...
  const loadProducts = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select('*')
        .or(
          `name.ilike.%${debouncedSearch}%,sku.ilike.%${debouncedSearch}%,barcode.eq.${debouncedSearch}`
        )
        .gt('quantity', 0)
        .order('name')
        .limit(20);
      ...
      if (status === 'granted') {
        ...
        Alert.alert(
          ...
          [
            ...
            {
              text: 'Open Settings',
              onPress: () => Camera.openSettingsAsync(),
            },
          ]
        );
      }
...
export default withErrorBoundary(SalesScreen, {
  errorTitle: 'Sales Error',
  errorMessage:
    'There was an issue with the sales interface. Please try again.',
  onError: (error, errorInfo) => {
    console.error('Sales screen error:', {
      error: error.message,
      stack: error.stack,
